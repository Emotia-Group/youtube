...
- ¿Ya lo usaste y quieres hacer un tipo concreto de video (un Short, un
  anuncio, un video con tu cara)? Ve directo al capítulo **4**.
- ¿Buscas algo puntual («cómo cambio la miniatura», «por qué salió caro»)?
  Usa el **buscador** que hay arriba del manual dentro del programa
  (📖 Manual de uso → 🔎) o el índice de la derecha.
...
![La pantalla de inicio del programa](docs/manual/01-nuevo-proyecto.png)
...
| **⚙ Configuración** | Claves, idioma, calidad, modelos y ahorro |
...
![Dónde se pegan las claves de API](docs/manual/09-configuracion-claves.png)
...
| `REPLICATE_API_TOKEN` | Crea las imágenes, los clips de video, la música y el lipsync | Sí, para tener imágenes de verdad | replicate.com → API tokens |
| `OPENAI_API_KEY` | Transcribe tu voz grabada y genera las imágenes donde se lee un texto | Sí, si narras tú | platform.openai.com → API keys |
| `ELEVENLABS_API_KEY` | Voces artificiales de máxima calidad | Opcional (solo si NO narras tú) | elevenlabs.io → perfil |
...
![El panel de un proyecto: los 11 pasos y la estimación](docs/manual/04-panel-del-proyecto.png)
...
![Los seis formatos y las plantillas de corto](docs/manual/02-formatos-y-plantillas.png)
...
(un plano cada ~13 segundos) e imágenes FLUX 1.1 Pro. La transcripción de tu
voz cuesta unos **$0.11**. Ver el capítulo 11 para bajarlo o subirlo a
conciencia.
...
> personaje. Con el modelo económico (SadTalker) son unos $1-4; con el de
> calidad cine (OmniHuman) pueden ser $18-29. **Estrategia: prueba siempre
> con SadTalker y deja OmniHuman para la versión final.**
...
4. Cuando el resultado te guste, **guarda ese estilo** (capítulo 5.6) y
...
![Las seis categorías de archivos](docs/manual/03-categorias-de-archivos.png)
...
### 5.2 Después de crear el proyecto: la pestaña 📎 Archivos

Ahí puedes **añadir o quitar material en cualquier momento**. Ten en cuenta
que al hacerlo se vuelve a analizar todo y los pasos se regeneran.

### 5.3 El elenco: que un personaje tenga siempre la misma cara
...
### 5.4 La estimación y el freno de gasto
...
### 5.5 El banco de elementos: material tuyo, gratis y para siempre
...
![El banco de elementos](docs/manual/13-banco-de-elementos.png)
...
### 5.6 Estilos de canal: la identidad, guardada y reutilizable
...
![La pestaña Concepto y el botón de guardar estilo](docs/manual/08-concepto-y-guardar-estilo.png)

**Gestionarlos:** menú **📺 Canales y estilos**. Ahí creas canales (para
agrupar), editas estilos o creas uno desde cero.

![Canales y estilos guardados](docs/manual/12-canales-y-estilos.png)
...
Mientras trabaja verás una barra de progreso, el porcentaje, el tiempo
restante estimado y, abajo, el registro en vivo.
...
![El video terminado con las 3 miniaturas y los 3 títulos](docs/manual/07-video-y-metadatos.png)
...
![El storyboard, escena por escena](docs/manual/05-storyboard.png)
...
![El editor de escenas](docs/manual/06-editor-de-escenas.png)
...
Se divide en cinco bloques. Al terminar, **💾 Guardar configuración**.

**a) 🔑 Claves de API** — capítulo 2.2.

**b) Estilo por defecto** — el que se propone en los proyectos nuevos:
...
![Los ajustes de video e idioma](docs/manual/10-configuracion-video.png)
...
**d) Módulo de integraciones (los modelos de IA).** Cada bloque muestra las
opciones con su precio y su punto fuerte y débil, y un punto verde ● o rojo
● según tengas la clave.

![La elección del modelo de imágenes](docs/manual/11-configuracion-modelos-imagen.png)
...
![El log de eventos](docs/manual/14-log-de-eventos.png)
...
~13 segundos). Ojo: el número de escenas manda sobre todo lo demás. Con el
ritmo por defecto (un plano cada 6 segundos) ese mismo video tendría ~180
escenas y las imágenes costarían el doble.

| Concepto | Costo aproximado |
|---|---|
| Inteligencia: guion, escenas, dirección de arte, control de calidad | $1-3 |
| **Imágenes con FLUX 1.1 Pro** (el equilibrio recomendado) | **$3.4-4.2** |
| Imágenes con FLUX dev | $2.0-2.5 |
| Imágenes con **FLUX schnell** (para pruebas) | $0.25-0.35 |
| Imágenes con SDXL Lightning (borradores) | $0.12-0.17 |
| Imágenes con gpt-image-1 | $5.9-21 ⚠ |
| Video generativo, 18 clips (Kling v1.6) | $2.3-6.3 ⚠ |
| Video generativo, 18 clips (Kling v2.1) | $4.5-9.0 ⚠ |
| Transcribir tu voz (18 min) | ~$0.11 |
| Voz artificial de OpenAI | $0.20-0.50 |
| Voz artificial de Edge | gratis |
| Música generada | $0.05-0.15 |
| Montaje, subtítulos, insertos, mapas y ambiente | gratis (se hacen en tu PC) |

### 11.2 Las siete reglas del ahorro

1. **Prueba barato, publica caro.** Genera el video completo con
   **FLUX schnell** (centavos). Cuando la estructura te convenza, cambia a
   FLUX 1.1 Pro y usa «Rehacer desde → Imágenes».
2. **Deja apagado el video generativo.** Con `providers.videogen.max_scenes`
...
3. **Párate en el punto de control.** Corregir el storyboard es gratis.
4. **Reutiliza estilos** guardados: evitas volver a analizar referencias.
5. **Llena el banco de elementos**: material gratis, tuyo, para siempre, y
   evita que se ilustre con IA algo que ya tienes.
6. **Baja el ritmo visual** de 6 a 8-10 segundos: menos imágenes, mismo
   contenido.
7. **Narra tú.** Es gratis (solo pagas la transcripción, centavos) y suena
   mejor que cualquier voz artificial.

### 11.3 Los interruptores de gasto, uno por uno

| Ajuste | Efecto en la factura |
|---|---|
| `providers.images.model` | **El que más pesa.** De schnell a Pro hay 13 veces de diferencia |
...
11. **Para los cortos, elige plantilla**: la estructura es la mitad del
    resultado.
12. **Usa 24 fps** (viene así en los estilos de cine): da aspecto de
    película. El estilo Moderno usa 30 fps a propósito.
...
| **Música por tramos** | Con tu biblioteca local y videos de más de 3 minutos, una pista distinta por tramo de intensidad, unidas con fundido (en vez de una sola pista plana) | `providers.music.multi_track` |
...
| **Pantalla dividida** | Dos imágenes a la vez cuando la escena compara algo (el antes y el después, dos versiones de un hecho) | automático |
| **Stickers de red social** | Imitación animada de los stickers de encuesta, pregunta o cuenta atrás, como recurso de retención (decorativos: no se pueden pulsar) | automático en formatos cortos |
...
| Costó más de lo estimado | Modelo caro o video generativo encendido | Revisa el capítulo 11.3 |
...
| El video se ve recortado | El proyecto se creó con otro formato | Crea un proyecto nuevo con el formato correcto (el formato se fija al crearlo) |
...
![La portada de la Torre de Control](docs/manual/15-torre-de-control.png)
...
![Los reportes comparando todos los canales](docs/manual/16-reportes-de-la-red.png)
